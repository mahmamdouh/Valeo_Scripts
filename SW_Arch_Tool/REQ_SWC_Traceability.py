...
Missed_SWC_DWI=[]
Missed_DWI=[]
Missed_SWRq_Justification_1 =[]
REQ_SWC_Traceability_Warning_Message ={
    "Message" : ""
}
        ...
        if workitems_list[i].type.id == "softwareRequirement":
            ...
            try:
                ...
                if SWC_Flag:
                    ...
                    Missed_SWRq_Justification = {
                        "ID": "",
                        "Justify": "",
                    }
                    Missed_SWRq.append(workitems_list[i].id)
                    if re.search("GenlSwRqrmt", workitems_list[i].title):
                        Missed_SWRq_Justification["Justify"] = "Generic SWREQ"
                    else:
                        Missed_SWRq_Justification["Justify"] = ""
                    Missed_SWRq_Justification_1.append(Missed_SWRq_Justification)
...
def KPI_data_write_SWC_LostIds(workbook1,data6, index, worksheet,infoList):
    ...
    # Data write.
    worksheet.write_url(ee, url2, string=data6)
...
def KPI_data_write_SWRQ_LostIds(workbook1,data5, index, worksheet,infoList):
    ...
    print(url2)
    print(data5)
    worksheet.write_url(aa, url2, string=str(data5))
    #worksheet.write(bb,str(data5["Justify"]))
    return worksheet
...
def KPI_data_write_DWI_LostIds(workbook1,data5, index, worksheet,infoList):
    ...
    # Data write.
    worksheet.write_url(cc, url2, string=data5)
    return worksheet
...
def polarion_query_SWCompvsREQ(infoList,polarion_object):
    ...
    query = "project.id:" + infoList[2] + " AND type: swComponent AND " + sql + " AND "+ infoList[5]

    #print("Receiving SW Components data...")
    workitems_list = polarion_object.tracker_webservice.service.queryWorkItems(
        query, "priority", ["id", "title", "type", "linkedWorkItems"])
    #("SWC WOrkitems : ", workitems_list)
...
def SWREQ_SWC_remove_Duplicates(Array_L):
    res = []
    for i in Array_L:
        if i not in res:
            res.append(i)
...
def BAD_IDS(workbook1,SWRQ_List , SWC_List ,DWI_List,worksheet ,infoList):
    SWRQ_List =SWREQ_SWC_remove_Duplicates(SWRQ_List)
    SWC_List = SWREQ_SWC_remove_Duplicates(SWC_List)
    DWI_List = SWREQ_SWC_remove_Duplicates(DWI_List)
    print("SWRQ_List",SWRQ_List)
    print("SWC_List", SWC_List)
    print("DWI_List", DWI_List)

    for i in range(len(SWRQ_List)):
        #print(str(SWRQ_List[i]))
        worksheet= KPI_data_write_SWRQ_LostIds(workbook1, SWRQ_List[i], (i+20), worksheet, infoList)
    for i in range(len(SWC_List)):
        #print(str(SWC_List[i]))
        worksheet= KPI_data_write_SWC_LostIds(workbook1, SWC_List[i], (i+20), worksheet, infoList)
    for i in range(len(DWI_List)):
        worksheet=KPI_data_write_DWI_LostIds(workbook1, DWI_List[i], (i+20), worksheet, infoList)
...
#infoList =['melmohta', 'Aliya$1994', 'VW_MEB_Inverter', '27_05_25_p332_and_q332', '24_11_1_26_p332', 'variant.KEY: (base\\+ base\\-)']



def REQ_SWC_Traceability_Runnable(infoList):
    ...
    data = [
        ['Total SWREQ', 'Not Covered SWREQ', 'Total DWI', 'Not Covered DWI', 'Total SWC', 'Not Covered SWC'],
        [0, 0, 0, 0, 0, 0],
    ]
    print("step 7")
    data[1][0] = KPI["Covered_SWREQ"]
