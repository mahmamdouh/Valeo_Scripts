import script.connectors.polarion_connector as c
from tqdm import tqdm
import xlsxwriter


def data_analysis(workitems_list, workbook, worksheet, infoList):
    index = 2
    for i in tqdm(range(len(workitems_list)), desc="Loading..."):
        LWI = []
        counter = 0
        # Get workitem type.
        type = workitem_type(workitems_list[i].id, infoList)
        if type.type.id == 'workpackage' or type.type.id == 'defect' or type.type.id == 'issue' or\
            ...
            try:
                ...
                for j in range(len(workitems_list[i].linkedWorkItemsDerived.LinkedWorkItem)):
                    ...
                    lwiData = workitem_type(lwiID, infoList)
                    ...
                    elif lwiData.type.id == 'workpackage' or lwiData.type.id == 'defect':
                        task_path(lwiID, infoList)
            except:
                pass
            # try:
            ...
            index = data_write(workitems_list, LWI, counter, i, index, worksheet)

    workbook.close()


def task_path(id, infoList):
...
def data_write(workitems_list, LWI, counter, i, index, worksheet):
    if counter > 0:
        # Identify data index.
        a = 'A' + str(index)
        b = 'B' + str(index)
        ...
        while j < (len(LWI)/3):
            ...
            j = j + 1
        if j > 1:
            cell_merge(index-j, index-1, url, workitems_list[i].id, workitems_list[i].title,
                       workitems_list[i].type.id, workitems_list[i].status.id)

    return index


def cell_merge(x, y, url, id, title, type, status):
...
def excel_open():
    ...
    workbook = xlsxwriter.Workbook('Outputs\Output.xlsx')
...
if __name__ == '__main__':
    ...
    data_analysis(workitems_list, workbook, worksheet, infoList)
    # Disconnet polarion.
