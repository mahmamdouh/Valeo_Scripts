...
def SF_SWC_get_work_items_Details_data(docs_content,info_list,SF_Array,output_workbook):
    docs_content_detail = {}
        ...
        while not_connected:
            try:
                ...
                work_item_list.append(polarion_object.tracker_webservice.service.queryWorkItems(
                    query, "priority", ["id", "title", "type","customFields.swc_major_version.KEY","customFields.swc_minor_version.KEY"]))
                not_connected = False
            except ConnectionError as e:
                # #print connection error message
        ...
        for element in work_item_list:
            for sub_element in element:
                if sub_element.type.id == "swComponent":
                    for Attributes in sub_element.customFields.Custom:
                        #print("Array",Attributes)
                        if Attributes['key'] == "swc_major_version":
                            Majoi_Version = str(Attributes['value'])
                        if Attributes['key'] == "swc_minor_version":
                            Minor_Version = str(Attributes['value'])
                            #"Major_Version": "",
                            #"Minor_Version": "",
                    ID = sub_element.id + " " + sub_element.title +"("+Majoi_Version+ "."+ Minor_Version+")"+" \n"
    ...
    #workbook.close()
    #print("Pass here 5")
    return docs_content_detail,SF_Array,workbook,worksheet3,work_item_list

def Create_Report(output_path,infoList):
    File_Name = "\System_Function_Report_" + infoList[3] + ".xlsx"
...
def SystemFunction_SWC_Report_Runnable(info_list):
    ...
    print("Step 6")
    docs_content_detail , SF_Array ,workbook,worksheet3,work_item_listEmpty = SF_SWC_get_work_items_Details_data(docs_content,info_list,SF_Array,output_workbook)
    print("Step 7")
