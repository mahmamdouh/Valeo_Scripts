...
def KPI_data_write_SWRQ_LostIds(workbook1,data5, index, worksheet,infoList):
    ...
    # Data write.
    worksheet.write_url(aa, url2, string=data5["ID"])
    worksheet.write(bb,data5["Justify"])
    return worksheet
...
def BAD_IDS(workbook1,SWRQ_List , SWC_List ,DWI_List,worksheet ,infoList):
    ...
    print("Justification",Missed_SWRq_Justification_1)
    ...
    for i in range(len(DWI_List)):
        worksheet=KPI_data_write_DWI_LostIds(workbook1, str(DWI_List[i]), (i+20), worksheet, infoList)
    ...
    while True:
        try:
            ...
            REQ_SWC_Traceability_Warning_Message["Message"] = "File Is already opened ! , please close Excel file "
            pass
