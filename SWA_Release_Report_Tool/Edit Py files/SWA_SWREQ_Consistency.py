...
def Compare_Document_Names(SWA_A , SWREQ_A ,Type):
    "Short_Name"
    ...
    for SWA_items in SWA_A:
        ...
        for SWRQ_items in SWREQ_A:
            ...
            if (SWA_Document_Title == SWRQ_Document_Title):
                ...
                Compare_SWA_SWAREQ_Document_Ids(SWA_items, SWRQ_items ,Type)

        else:
            pass
    return SWA_SWREQ_Total_KPI , Document_Not_have_match_name
...
def Compare_All_Document_Inconsistency_Missed_Ids(SWA_A , SWREQ_A,Document,Algo):
    Found_Flag = 0
    Found_Flag_array = []
    ...
    return NOT_Found_Flag_array,NOT_Found_Flag_array_Data,Found_Flag_array

def Filter_Total_Missing_Ids(NOT_Found_Flag_array,NOT_Found_Flag_array_Data,Found_Flag_array,Current_Algo):
...
def Compare_SWA_SWAREQ_Document_Ids(SWA_IDs ,SWRQ_IDs ,Type):
...
def Generate_Temp_Report(info_list,SWA_Array, SWAREQ_Array):
    # Create / Open Excel file.
    File_Name = "\Outputs\SWA_SWREQ_Consistency_Temp_" + info_list[3] + ".xlsx"
    workbook1 = xlsxwriter.Workbook(File_Name)
...
def Compare_All_Document_Not_Founded_In_Other_Baseline(Document_Short_name ,SWA_A , SWREQ_A,Document,Algo):
    Found_Flag = 0
    Current_Comparing=""
    Found_Flag_array = []
    NOT_Found_Flag_array = []
...
def Generate_Inconsistency_Report(SWA_SWREQ_Total_KPI2,workbook1, worksheet3, index):
    for items in SWA_SWREQ_Total_KPI2:
        ...
        ##print("Arch_Document ", items["SWA_DOC"])
        ##print("REQ_Document ", items["SWREQ_DOC"])
        ##print("Message ", items["Message"])
        ##print("Missing_Ids ", items["Missing_Ids"])
        Data.append(str(items["SWA_DOC"]))
        Data.append(str(items["SWREQ_DOC"]))
...
if __name__ == '__main__':
    ...
    #############################################################33
    ### Mised IDS aLgo
    NOT_Found_Flag_array, NOT_Found_Flag_array_Data, Found_Flag_array=Compare_All_Document_Inconsistency_Missed_Ids(SWA_Missed_IDs_With_Document, SWAREQ_Array, "SWA","Algo1_1")
    Filter_Total_Missing_Ids(NOT_Found_Flag_array, NOT_Found_Flag_array_Data, Found_Flag_array,"Algo1_1")

    #print("(6)")
    index=1
    #print("(7) save Data ")
    NOT_Found_Flag_array, NOT_Found_Flag_array_Data, Found_Flag_array = Compare_All_Document_Inconsistency_Missed_Ids(SWREQ_Missed_IDs_With_Document, SWA_Array, "SWREQ","Algo1_2")
    #print("(8)")
    ##print("Current Indix ===========:", index)
    ...
    #print("(10)")
    NOT_Found_Flag_array,NOT_Found_Flag_array_Data,Found_Flag_array= Compare_All_Document_Not_Founded_In_Other_Baseline(SWA_Document_Not_have_match_name,SWA_Array, SWAREQ_Array, "SWA","Algo2_1")
    ...
    #print("(7)")
    #Missed_IDs_With_Document_Report = remove_Duplicates(Missed_IDs_With_Document_Report)
